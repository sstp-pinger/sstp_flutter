...
internal class IPTerminal(private val bridge: ClientBridge) {
    ...
    private fun setIPv4BasedRouting() {
        if (isDefaultRouteAdded) {
            ...
            // Let LAN traffic bypass the tunnel so the in-app SOCKS proxy can
            // serve other devices on the same Wi-Fi: their reply packets must
            // return via the physical interface, not get captured by the VPN's
            // 0.0.0.0/0 default route. Only meaningful when private addresses
            // aren't deliberately routed into the tunnel. excludeRoute is
